...
namespace DMS_CPMS.Controllers
{
    ...
    public class PatientController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreatePatientViewModel model)
        {
            ...
            {
                const long MaxFileBytes = 10L * 1024 * 1024; // 10 MB
                var currentUserId = _userManager.GetUserId(User);
                var file = model.UploadedFile;

                if (file.Length > MaxFileBytes)
                {
                    TempData["Warning"] = "Patient record was created successfully, but the attached file exceeds the 10 MB size limit and was not uploaded.";
                }
                else if (IsAllowedFileType(file.FileName))
                {
        ...
        private bool IsAllowedFileType(string fileName)
        {
            ...
            return FileUploadConstants.AllowedExtensions.Contains(extension);
        }

...
function pickerCallback(data) {
    ...
    if (data.action === google.picker.Action.PICKED) {
        ...
        if (file) {
            ...
            // Do NOT store access token in the DOM — keep it only in the JS variable
...
/**
 * Handles upload form submission — supports both device (multi-file) and Google Drive.
...
async function handleUploadFormSubmit(e) {
    ...
    try {
        ...
        if (sourceType === 'googledrive') {
            ...
            // ── Standard device upload (supports multiple files) ──
            const fileInput = document.getElementById('uploadFileInput');
            ...
            if (fileInput.files.length > 5) {
                showNotification('You can upload a maximum of 5 files at a time.', 'error');
                return;
            }
...
function resetUploadModal() {
    ...
    const validationMsg = document.getElementById('nestedFileValidationMsg');
    if (validationMsg) validationMsg.style.display = 'none';
    const filesList = document.getElementById('nestedSelectedFilesList');
